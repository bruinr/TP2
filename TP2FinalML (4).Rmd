...
link to author's kaggle submission:
...
link to author's kaggle submission:
https://www.kaggle.com/anwarmohammad/housing-prices-prediction-using-r-anwar
...
*  imported  the data and set up the training and test data
*  analyzed this data to determine how to deal with missing data.  
*  Set the seed for reproducibility. Skewness is a measure of the asymmetry of the probability distribution of a real-valued random variable about its mean. 
*  inputted the missing values using the missforest package which runs a randomForest on each variable and used the observed part to predict  any missing  values. 
*  verified the skewness of the dataset features and took the log transformation of the features for which skewness is more than 0.75. 
*  used the dummervars() function in the caret package for one-hot encoding of categorical variables. 


**Ridge and Lasso Summary**


* created Ridge and Lasso models using the repeatedcv() function and set the number to 10 with 10 repeats
  + setting the method to repeatedcv() which controls the type of resampling and is used to specify repeated k-fold cross-validation. 



**Ridge Model In Depth Analysis**

*  created the model using the trainControl() function and set the method to “repeatedcv”, number to 10, and repeats to 10
*  created a lambas variable using the seq() function.
*  created the ridge model using glmnet, set the metric to RMSE, and expanded the grid by setting alpha to 0 and lambda to the lambas variable that was recently created. 
*  iterated through a sequence of increasing lambdas that were declared in the lambdas variable to utilize the mode for further interpretation of finding the optimal lambda. Note: The best lambda, however, is not disclosed or even implemented. 


*  the user attempted to verify the accuracy of the ridge model by calling the results variable
  + this shows error metrics for the variables of the model
  + meant to help the user pick the best lambda based on the optimal RMSE error metric
  + the user did not disclose his finding on the given output. 
...
* looked at the contribution of each variable to make a prediction to show glmnet variable importance
...
* made predictions on the test set of the model to file for kaggle. 
...
* predict the sale price on the dataset using the train() function. 
  + implemented through the glmnet() method and by setting the metric to “RMSE.” 
  + The user also uses the expand.grid() function by setting alpha to 1 and setting up a lambda grid. The difference between ridge and lasso regression is that lasso tends to change coefficients to absolute zero as compared to Ridge which never sets the value of coefficient  to absolute zero. 
  + incorrectly implements this expand.grid() function which is meant to help identify the best lambda, which he never does.
This will be elaborated in the critique.

* verifies the accuracy of the model by calling the results variable in the lasso model
  + The intention is to assess RMSE as the error metric, which seems to stay the same as lambda increases
  + This shows that the lasso model is not efficient in reducing error metric on the data set
  + This will be elaborated in the critique. 
...
* Sample of the accuracy assessing the error metrics of the model, which we see of the Lasso Model:
...
* analyzes the contribution of each variable to make a prediction using the varImp() function
 

  The Following Top 20 variables shown out of 289 in descending order. 

<center>
![Lasso Model Variable Importance Sample Outputt](LassoVImportanceSample.png){width=25%}
</center>

* Lasso predictions of the test dataset using calling $result
...
* assessed the accuracy of the models by setting the metric to “RMSE"
  + called $results of the given model
  + intention of assessing this information to help pick the best lambda based on the optimal RMSE error metric;
  + the user did not disclose his finding on the given outputs.
...
* In the lambda model when the user verifies the accuracy,
  + the user’s  assessment of RMSE error metric shows to stay the same as lambda
increases
  + This shows that the lasso model is not efficient in reducing error metric on the data set. 
...
* Analyzed the contribution of each variable within the model by using the varImp() function to 
assess the importance of each variable. 

  + Notice how the importance values also do not say anything about the relationship 
...
* makes a prediction on the test set using the predict() function.
...
* created a model and sets the method to “lm” and metric to “RMSE”as the error metric

<center>
![Linear Regression Model Code](LRCode.png){width=60%}
</center>

* verified the accuracy of the model by calling the results variable from the model in order to assess the model’s error metrics. 
...
* cross-validation technique repeatedcv() 

* use of the RMSE error metric within his/her models. 

* does not implement the best lambda, and only prints the results to attempt at analyzing the accuracy of the model that implements the training data rather than running through data. 


**Data Preprocessing and Feature Engineering**


* poor strategy for improving the quality of the data by assessing the skewness before ever attempting to improve the quality of the data
  + Assessing the skewness should be done __after__ data preprocessing.


* initially separated  the numeric and character values in the training data  he only then assessed the missing values within the numeric data using the dummyvars() function that predicted NA values in the numeric category. 

* By separating the numeric and character attributes into separate vectors, the user created models that do not analyze attributes with characters at all
  + This implementation created inaccurate data, where more specific or intuitive approaches could have been utilized.  
...
*  Need to include attributes with characters, which is a vast portion of significant variables needed for the regression model/models.
*  There are  numeric values in certain records that can manually be replaced with a “0,” such as the attributes “LotFrontage”, “MasVnrArea”, and “BsmtFinSF1”. The function for implementing this looks like this:
...
* change the values of NA’s that have actual meanings, no meanings at all, and others that have a more intuitive approach:

* the NA records in attributes with actual meanings 
  + such as in the Alley attribute can be assumed to actually mean “No,” with no meaning at all such as in the KitchenQual attribute can be replaced with the most common value of this feature
* more intuitive approach 
  + the feature GrgYrBlt, we can assume that the year the garage was built is either the year the house was built or if the house was remodeled we could take the average of both years, given a 50/50 probability that it could be either/or. 
* Another way to improve the quality of the data is to change empty values to “No.” 

By this point, it can be worth assessing the NA values in the dataset. 
  
* could identify outliers in the dataset by running a linear regression to predict the sale price based on the predictors in the dataset
  + once this is done, the user could plot the linear regression model to assess the cook’s distance plot for outliers to be removed.
  + can also plot these features against the target variable to individually assess the predictor variables. 
  + The user could either remove the outliers or transform them to the mean of each variable. 

* assess character variables into continuous numerical variables
  + Some character variables require a transformation to ranking/ binary values, and others require completely new features. 

*Binary Transformation:

  + For example: the ExterQual variable requires a ranking  
  
*New Features to be added

  + For example: the user could create features to determine whether the house was remodeled after it was sold 
  
  + if so, alter the data. 
  
* The user could further create features

  + For example: to analyze whether the house was remodeled after it was sold, to assess the number of years since the house 
  was remodeled and sold to disclose how old the house is the time since it was last sold, to calculate the total surface area
  of the house. Furthermore, the user should create a calculated field to determine hot months for homes to be sold as well as 
  calculate the attributing month the individual house was sold in with a given value. Some character features solely require 
  binarizing, because they are primarily unique. For instance, LotShape would be binarized, because it either has a regular shape 
  or it does not. The user could also binarize area-related features.

**Model Errors**

* Needed to obtain the lasso or ridge regression fit with:
```{r, results=FALSE}
 #cvfit <- cv.glmnet(X, y)
```


*  Needed to get the best lambda value of lambda out
```{r, results=FALSE}
...
*  Could even even asses the best mse 
```{r, results=FALSE}
 # mse <- cvfit$cvm[cvfit$lambda == lambda.min]

```
	
*  User’s error metric assessment.  The user runs tests and produces an output showing multiple items.  
  + However, they don’t say which items in this output are the measure that they’re focusing on. 
  + It can be inferred that RMSE is likely the error measure of choice, but the user does not specify which model produces 
  the best error measure for accuracy analysis.  
  + This is a critical area for improvement since it makes it difficult to assess the user’s results.
